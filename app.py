...
# ──────────────────────────────────────────
with tab6:
    st.subheader("📝 追蹤與損益分析")

    USER_PICKS_FILE = Path("user_picks_records.json")
    ...
    if _track_mode.startswith("💰"):
        ...
        if user_picks:
            period_map_pk = {d.period: d for d in draws}
            period_list_pk = sorted(period_map_pk.keys())

            _updated = False
            for up in user_picks:
                if up["實際開獎"] is None:
                    for p in period_list_pk:
                        if p > up["依據期數"] and p in period_map_pk:
                            d = period_map_pk[p]
                            up["實際開獎"] = d.numbers
                            ...
                            break
            if _updated:
                USER_PICKS_FILE.write_text(json.dumps(user_picks, ensure_ascii=False, indent=2))

            settled_picks = [p for p in user_picks if p["實際開獎"] is not None]
            total_cost = len(settled_picks) * 50
            total_revenue = sum(p["獎金"] for p in settled_picks)
            net_profit = total_revenue - total_cost
            roi = (net_profit / total_cost * 100) if total_cost > 0 else 0

            _pc1, _pc2, _pc3, _pc4 = st.columns(4)
            _pc1.metric("累積投入成本", f"${total_cost:,}")
            _pc2.metric("累積獲得獎金", f"${total_revenue:,}")
            _pc3.metric("淨損益 (Net P&L)", f"${net_profit:,}", f"{roi:.1f}% ROI")
            _pc4.metric("待開獎注數", f"{len(user_picks) - len(settled_picks)} 注")

            if settled_picks:
                cumulative_pnl = []
                current_pnl = 0
                for p in settled_picks:
                    current_pnl += (p["獎金"] - p["成本"])
                    cumulative_pnl.append(current_pnl)

                _last_color = "#4ECDC4" if cumulative_pnl[-1] >= 0 else "#FF6B6B"
                fig_pnl = go.Figure()
                fig_pnl.add_trace(go.Scatter(
                    x=[p["時間"] for p in settled_picks],
                    y=cumulative_pnl,
                    mode='lines+markers',
                    line=dict(color=_last_color, width=3),
                    marker=dict(size=6),
                    fill='tozeroy',
                    fillcolor=f"rgba(78,205,196,0.1)" if cumulative_pnl[-1] >= 0 else "rgba(255,107,107,0.1)"
                ))
                fig_pnl.add_hline(y=0, line_dash="dash", line_color="#888")
                fig_pnl.update_layout(**_dark_layout("模擬損益資產曲線", height=280, yaxis_title="累積淨利 (NTD)"))
            ...
            def _pnl_color(row):
                h = row["_hit"]
                if h == -1: return ["color:#888"] * len(row)
                if h >= 4: return ["background:rgba(255,107,107,0.15);color:#D32F2F;font-weight:800"] * len(row)
                if h >= 2: return ["background:rgba(78,205,196,0.15);color:#00796B;font-weight:700"] * len(row)
                return ["color:#555"] * len(row)

            st.dataframe(
                pd.DataFrame(pick_rows).style.apply(_pnl_color, axis=1).hide(axis="columns", subset=["_hit"]),
                width="stretch", hide_index=True
            )
        else:
            st.info("尚無自選紀錄，請在上方輸入號碼模擬下注。")
        ...
        if records:
            ...
            for r in reversed(records[-20:]):
                ...
                display_rows.append({
                    ...
                    "_hit": hit5 if hit5 is not None else -1, "_pending": pending,
                })

            def _color_hit(row):
                h = row["_hit"]
                if row["_pending"]: return ["color:#888"] * len(row)
            ...
            st.dataframe(
                pd.DataFrame(display_rows)[show_cols + ["_hit","_pending"]].style.apply(_color_hit, axis=1).hide(axis="columns", subset=["_hit","_pending"]),
                width="stretch", hide_index=True,
            )
            if hit_data:
                fig_h = go.Figure(go.Bar(
                    ...
                    text=[f"{r['5碼命中']}/5" for r in hit_data[-20:]], textposition="outside",
                ))
            ...
            st.markdown("<div style='background:#f8f9fa;border:1px solid #eee;border-radius:16px;padding:1.5rem;text-align:center;color:#999'>尚無推薦紀錄</div>", unsafe_allow_html=True)

        st.markdown("")
        if st.button("儲存本次系統推薦", type="primary", key="save_sys_rec"):
            save_record(latest.period, rec.top5, rec.top6, rec.top7, rec.killed)
            st.success("已儲存！開獎後可於此處比對命中率。")

        # ── 策略篩選回測 ──
        st.markdown("---")
        st.markdown("#### 🎯 策略篩選回測（自訂條件 → 查看歷史命中率）")
        with st.expander("設定策略條件", expanded=False):
            ...
            if st.button("▶ 執行策略回測", key="strat_bt"):
                ...
                for i in range(1, len(draws)):
                    d = draws[i]; nums = d.numbers; ok = True
                    if _strat_odd != "不限" and sum(1 for n in nums if n % 2 == 1) != int(_strat_odd): ok = False
                    if _strat_big != "不限" and sum(1 for n in nums if n >= 20) != int(_strat_big): ok = False
                    if not (_sum_lo <= sum(nums) <= _sum_hi): ok = False
                    if _strat_repeat and not (set(nums) & set(draws[i-1].numbers)): ok = False
                    if _strat_consec:
                        s = sorted(nums)
                        if not any(s[j+1]-s[j] == 1 for j in range(len(s)-1)): ok = False
                    if ok: _match += 1
                    _total += 1
                st.session_state["strat_result"] = {"match": _match, "total": _total, "rate": _match / _total if _total else 0}

        if "strat_result" in st.session_state:
            _r = st.session_state["strat_result"]
            _sa, _sb, _sc_col = st.columns(3)
            _sa.metric("符合條件期數", f"{_r['match']} 期")
            _sb.metric("總回測期數", f"{_r['total']} 期")
            _sc_col.metric("歷史出現率", f"{_r['rate']:.1%}")
